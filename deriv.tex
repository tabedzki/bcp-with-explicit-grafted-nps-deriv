\documentclass{article}
  ...
  The graft site distribution is defined as a thin shell at the surface of the
    ...
    % Changing the expression for Gamma to more closely resemble Jason's paper
  \begin{align*}
  ...
  \begin{align*}
    ...
    =
      ...
      \right) \\
              &\times
        \exp\left(
          - i \int d \mathbf{r} \hat{\rho}_{P}  w_+ %possibly? 
        \right)
  \\
    =
      &\exp \left(
    ...
    =
      &\exp \left(
        ...
        - \int d \mathbf{r} \hat{\rho}_{GC} w_C
         - \int d \mathbf{r} \hat{\rho}_{P} w_+ %possibly? 
      \right) \\
  \end{align*}
  where
  \begin{align*}
  ...
  \begin{align*}
    \mathcal{H}[w_+, w_{AB}^{(\pm)},w_{BC}^{(\pm)},w_{AC}^{(\pm)}] =&
      ...
      \right) \\
      &  
      % \left( - i \int d \mathbf{r} \hat{\rho}_{P}  w_+ %possibly? 
      % \right)
      - n_D \log Q_D 
      - n_G \int d \mathbf{r} \sigma_G(\mathbf{r}) 
        \ln(q_G[\mathbf{r};\omega_G]) 
 ...
 Below is the derivation of the system for a variable version of graft chains and such.
  ...
  % We do not have to do an expansion here since there are no imaginary terms here
  % This gives us
  % \begin{align*}
  %   \left. \hat{w}_{AB}^{(+)} \right|_{t+1} =
  %     \left. \hat{w}_{AB}^{(+)} \right|_t - \lambda \left[
  ...
  % Thus, given a 3-field model using $z_{D,3}$, the corresponding value of
